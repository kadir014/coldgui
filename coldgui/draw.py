...
def dashed_line(surface, color, start_pos, end_pos, width=1, dash_length=10, fixed=False):
    ...
    for index in range(0, int(length / dash_length), 2):
        ...
        if fixed: end = origin + (slope * index * dash_length)
        else: fixed: end = origin + (slope * (index + 1) * dash_length)
